...
describe('TrainerPage page', () => {
    describe('Initial elements', () => {
        it('Renders 8x8 chessboard with pieces', () => {
            ...
            expect(
                screen.queryAllByRole('img', { name: /pawn$/i })
            ).toHaveLength(16);
            expect(
                screen.queryAllByRole('img', { name: /rook$/i })
            ).toHaveLength(4);
            expect(
                screen.queryAllByRole('img', { name: /knight$/i })
            ).toHaveLength(4);
            expect(
                screen.queryAllByRole('img', { name: /bishop$/i })
            ).toHaveLength(4);
            expect(
                screen.queryAllByRole('img', { name: /queen$/i })
            ).toHaveLength(2);
            expect(
                screen.queryAllByRole('img', { name: /king$/i })
            ).toHaveLength(2);
        });
